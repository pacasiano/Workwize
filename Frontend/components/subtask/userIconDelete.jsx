...
function Delete({user}) {
    ...
    return (
        <div onMouseEnter={showSet} onMouseLeave={showSet} className={`relative transition-all rounded-full  h-8 text-center flex flex-row items-center justify-center hover:scale-105 hover:cursor-pointer bg-orange-300`}>
            <div className={`flex flex-row justify-evenly items-center gap-1 w-8 px-2 ${show && "w-12"} font-bold font-mono text-black`}>{user.username.charAt(0) || ""}
            {show && 
